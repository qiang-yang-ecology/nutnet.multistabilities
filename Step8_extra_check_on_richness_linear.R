# ---- Directional richness response vs enrichment intensity 

## ----------------------------------------------------------
## 1. Recreate df.tidy.coverage and control selection 
## ----------------------------------------------------------

# df.tidy must already be loaded from df.tidy.Rdata
# Only plots with coverage data
  ...
  mutate(
    results = map2(
      ...
      .f = function(dat1, dat2) {
        ...
        # richness per year per group
        dat.nested <- dat %>%
          group_by(year_trt, g) %>%
          summarise(richness = n_distinct(Taxon), .groups = "drop")
        
        # Log response ratio per year
        lRR.richness <- dat.nested %>%
          pivot_wider(names_from = g, values_from = richness) %>%
...
pdf(paste0(getwd(), "/myfigure/LRR_model_tables_N_and_P (richness).pdf.pdf"), width = 7, height = 7)
...
pdf(paste0(getwd(), "/myfigure/Predicted effects of nutrient-addition intensity and time on directional richness responses (LRR) under N and P enrichment.pdf"), width = 7, height = 5)
print(p_lrr_pred)
dev.off()
